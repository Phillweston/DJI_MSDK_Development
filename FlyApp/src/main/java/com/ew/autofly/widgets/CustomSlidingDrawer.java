...
public class CustomSlidingDrawer extends ViewGroup {
    ...
    public CustomSlidingDrawer(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.CustomSlidingDrawer, defStyleAttr, defStyleRes);
        ...
        int handleId = a.getResourceId(R.styleable.CustomSlidingDrawer_handleLayout, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(R.styleable.CustomSlidingDrawer_contentLayout, 0);
        if (contentId == 0) {
