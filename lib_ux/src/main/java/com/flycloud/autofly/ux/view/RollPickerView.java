...
public class RollPickerView extends View {
    ...
    public interface OnValueChangeListener{
        void onValueChange(RollPickerView picker, int oldVal, int newVal);
    }

    public interface OnValueChangeListenerRelativeToRaw{
        void onValueChangeRelativeToRaw(RollPickerView picker, int oldPickedIndex, int newPickedIndex,
                                        String[] displayedValues);
    }

    public interface OnValueChangeListenerInScrolling{
        void onValueChangeInScrolling(RollPickerView picker, int oldVal, int newVal);
    }
    ...
    public interface OnScrollListener {
        ...
        void onScrollStateChange(RollPickerView view, int scrollState);
    }
    ...
    public RollPickerView(Context context) {
        super(context);
        init(context);
    }
    public RollPickerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttr(context, attrs);
        init(context);
    }
    public RollPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttr(Context context, AttributeSet attrs){
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RollPickerView);
        int n = a.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = a.getIndex(i);
            if(attr == R.styleable.RollPickerView_npv_ShowCount){
                mShowCount = a.getInt(attr, DEFAULT_SHOW_COUNT);
            }else if(attr == R.styleable.RollPickerView_npv_DividerColor){
                mDividerColor = a.getColor(attr, DEFAULT_DIVIDER_COLOR);
            }else if(attr == R.styleable.RollPickerView_npv_DividerHeight){
                mDividerHeight = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_HEIGHT);
            }else if(attr == R.styleable.RollPickerView_npv_DividerMarginLeft){
                mDividerMarginL = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_MARGIN_HORIZONTAL);
            }else if(attr == R.styleable.RollPickerView_npv_DividerMarginRight){
                mDividerMarginR = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_MARGIN_HORIZONTAL);
            }else if(attr == R.styleable.RollPickerView_npv_TextArray){
                mDisplayedValues = convertCharSequenceArrayToStringArray(a.getTextArray(attr));
            }else if(attr == R.styleable.RollPickerView_npv_TextColorNormal){
                mTextColorNormal = a.getColor(attr, DEFAULT_TEXT_COLOR_NORMAL);
            }else if(attr == R.styleable.RollPickerView_npv_TextColorSelected){
                mTextColorSelected = a.getColor(attr, DEFAULT_TEXT_COLOR_SELECTED);
            }else if(attr == R.styleable.RollPickerView_npv_TextColorHint){
                mTextColorHint = a.getColor(attr, DEFAULT_TEXT_COLOR_SELECTED);
            }else if(attr == R.styleable.RollPickerView_npv_TextSizeNormal){
                mTextSizeNormal = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_NORMAL_SP));
            }else if(attr == R.styleable.RollPickerView_npv_TextSizeSelected){
                mTextSizeSelected = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_SELECTED_SP));
            }else if(attr == R.styleable.RollPickerView_npv_TextSizeHint){
                mTextSizeHint = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_HINT_SP));
            }else if(attr == R.styleable.RollPickerView_npv_MinValue){
                mMinShowIndex = a.getInteger(attr, 0);
            }else if(attr == R.styleable.RollPickerView_npv_MaxValue){
                mMaxShowIndex = a.getInteger(attr, 0);
            }else if(attr == R.styleable.RollPickerView_npv_WrapSelectorWheel){
                mWrapSelectorWheel = a.getBoolean(attr, DEFAULT_WRAP_SELECTOR_WHEEL);
            }else if(attr == R.styleable.RollPickerView_npv_ShowDivider){
                mShowDivider = a.getBoolean(attr, DEFAULT_SHOW_DIVIDER);
            }else if(attr == R.styleable.RollPickerView_npv_HintText){
                mHintText = a.getString(attr);
            }else if(attr == R.styleable.RollPickerView_npv_AlternativeHint){
                mAlterHint = a.getString(attr);
            }else if(attr == R.styleable.RollPickerView_npv_EmptyItemHint){
                mEmptyItemHint = a.getString(attr);
            }else if(attr == R.styleable.RollPickerView_npv_MarginStartOfHint){
                mMarginStartOfHint = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_MARGIN_START_OF_HINT_DP));
            }else if(attr == R.styleable.RollPickerView_npv_MarginEndOfHint){
                mMarginEndOfHint = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_MARGIN_END_OF_HINT_DP));
            }else if(attr == R.styleable.RollPickerView_npv_ItemPaddingVertical){
                mItemPaddingVertical = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_ITEM_PADDING_DP_V));
            }else if(attr == R.styleable.RollPickerView_npv_ItemPaddingHorizontal){
                mItemPaddingHorizontal = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_ITEM_PADDING_DP_H));
            }else if(attr == R.styleable.RollPickerView_npv_AlternativeTextArrayWithMeasureHint){
                mAlterTextArrayWithMeasureHint = a.getTextArray(attr);
            }else if(attr == R.styleable.RollPickerView_npv_AlternativeTextArrayWithoutMeasureHint){
                mAlterTextArrayWithoutMeasureHint = a.getTextArray(attr);
            }else if(attr == R.styleable.RollPickerView_npv_RespondChangeOnDetached){
                mRespondChangeOnDetach = a.getBoolean(attr, DEFAULT_RESPOND_CHANGE_ON_DETACH);
            }else if(attr == R.styleable.RollPickerView_npv_RespondChangeInMainThread){
                mRespondChangeInMainThread = a.getBoolean(attr, DEFAULT_RESPOND_CHANGE_IN_MAIN_THREAD);
            }else if (attr == R.styleable.RollPickerView_npv_TextEllipsize) {
        ...
        if(currPickedIndex != mPrevPickedIndex && mRespondChangeOnDetach){
            try {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(RollPickerView.this, mPrevPickedIndex + mMinValue, currPickedIndex + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(RollPickerView.this, mPrevPickedIndex, currPickedIndex, mDisplayedValues);
                }
            }catch (Exception e){
                e.printStackTrace();
            }
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange){
        ...
        if(oldVal != newVal){
            if(respondChange == null || !(respondChange instanceof Boolean) || (Boolean) respondChange) {
                if(mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(RollPickerView.this, oldVal + mMinValue, newVal + mMinValue);
                }
                if(mOnValueChangeListenerRaw != null){
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(RollPickerView.this, oldVal, newVal, mDisplayedValues);
                }
            }
        }
